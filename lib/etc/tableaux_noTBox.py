...
class TNode:
    ...
    def __init__(self):
        ...
        self.abox = []
        self.inactive_abox = []  # insieme di assiomi
...
class Tableaux:
    ...
    def calculate_clashes(self, node):
        abox = node.inactive_abox + node.abox
        node.clashes = [(t1, t2) for t1 in abox if 'neg' in t1.keys()
                        for t2 in abox if t2 == assign(t1['neg'], t1['args'])]
    ...
    def expand_node(self, node):
        ...
        if len(and_terms) > 0:  # and rule
            ...
            new_node.abox = [assign(t, a['args']) for a in and_terms for t in a['and']
                             if assign(t, a['args']) not in node.abox + node.inactive_abox] + \
                            exists_terms + forall_terms + or_terms + concepts + relations
            new_node.inactive_abox = node.inactive_abox + and_terms
            ...
            new_node.inactive_abox = node.inactive_abox + exists_terms
            ...
            for e in exists_terms:
                ...
                cz = set([con['args'][0] for con in node.abox + node.inactive_abox if concept_equal(c, con)])
            ...
            new_node.inactive_abox = node.inactive_abox + forall_terms
            ...
            for f in forall_terms:
                ...
                cy = set([con['args'][0] for con in node.abox + node.inactive_abox if concept_equal(c, con)])
            ...
            new_node.inactive_abox = node.inactive_abox + [or_term]
            new_node1.inactive_abox = node.inactive_abox + [or_term]
